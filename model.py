...
class TableGan(object):
    def __init__(self, sess, input_height=108, input_width=108, crop=True,
        ...
        if not self.y_dim: #y_dim이 None이면 작동을 한다. 
            self.g_bn3 = batch_norm(name='g_bn3')
    ...
    def build_model(self):
        ...
        self.y = tf.placeholder( #placeholer는 외부 데이터를 그래프에 공급하기 위해 사용, tensor를 만들어준다. float32 유형의 [batch_size, 차원] 형태.
            tf.float32, [self.batch_size, self.y_dim], name='y')

        self.y_normal = tf.placeholder( #y_normal은 차원이 1인 경우.
