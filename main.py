...
flags.DEFINE_float("beta1", 0.5, "Momentum term of adam [0.5]") #SGD+momentum이용.
flags.DEFINE_integer("train_size", sys.maxsize, "The size of train images [np.inf]")
flags.DEFINE_integer("y_dim", 2, "Number of unique labels") #The number of Labels. real, fake
...
flags.DEFINE_integer("output_height", 16, "The size of the output images to produce [64]") #input이랑 output의 크기가 같기 때문에 Padding 적용
...
flags.DEFINE_boolean("shadow_gan", False, "True for loading fake data from samples directory[False]") #생성한 이미지를 가져오는 것
